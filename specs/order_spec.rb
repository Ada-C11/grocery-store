...
describe "Order Wave 2" do
  ...
  describe "Order.find" do
    ...
    it "Returns nil for an order that doesn't exist" do
      # TODO: Your test code here!
      unfound_order = Order.find(9999999)

      expect(unfound_order).must_be_nil
    end
  end

  xdescribe "Order.find_by_customer" do
    it "Can find the orders placed by one customer" do
      cust_orders = Order.find_by_customer(25)

      expect(cust_orders[0]).must_be_kind_of Order
      expect(cust_orders.length).must_equal 6
      # customer 25's order numbers are 1, 20, 28, 51, 72, 95
    end
  end
end
