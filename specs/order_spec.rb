...
describe "Order Wave 1" do
  ...
  describe "#remove_product" do
    ...
    it "Raises an ArgumentError if product is not in products" do
      products = {"banana" => 1.99, "cracker" => 3.00, "pickle" => 1.50}
      order = Order.new(1337, products, customer)
      expect {
        order.remove_product("eggplant")
      }.must_raise ArgumentError
    end
  end
end
...
describe "Order Wave 2" do
  ...
  describe "Order.find_by_customer" do
    it "returns an array with the correct number of Order instances" do
      # orders = Order.all
      customer_orders = Order.find_by_customer(25)

      expect(customer_orders.length).must_equal 6
      customer_orders.each do |order|
        expect(order).must_be_kind_of Order
      end
    end

    it "returns nil if there are no orders for a customer" do
      customer_orders = Order.find_by_customer(150)
      expect(customer_orders).must_be_nil
    end
  end
end
